...
type Props = {
  provider: "google" | "microsoft" | "apple" | "x" | "instagram" | "facebook";
  patientId?: string | null;
  linkToken?: string | null;
};

export default function SocialSignInButton({ provider, patientId, linkToken }: Props) {
  const { instance } = useMsal();

  async function signIn() {
    if (provider === "microsoft") {
      // Microsoft MSAL authentication with popup-first approach
      if (!instance) {
      ...
      try {
        ...
        // If popup succeeded, acquire token and call link endpoint (if we have patientId/linkToken)
        if (patientId && linkToken) {
          ...
          if (!resp.ok) {
            ...
            console.info("Microsoft popup login and linking succeeded");
          }
        } else {
          console.info("Microsoft popup login succeeded (no linking data available yet)");
        }
      } catch (popupErr) {
        console.warn("Microsoft popup failed or blocked, falling back to redirect", popupErr);

        // Save resume state so redirect flow knows what to link (if we have the data)
        if (patientId && linkToken) {
        ...
        try {
          ...
          // The browser will navigate away — our resume hook will pick up after redirect return
        } catch (redirectErr) {
          console.error("Microsoft loginRedirect failed", redirectErr);
      ...
      console.log(`${provider} authentication not yet implemented`);
    }
  }
