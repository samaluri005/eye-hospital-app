...
- **Modern Registration Flow**: Offers a clean, progressive signup experience with a 5-step journey (Profile → Verification → Complete Profile → MFA Setup → Complete). Step 1-2 during initial registration (Personal + Contact info), Step 3 for optional extended profile, Step 4 for optional MFA, Step 5 completion. Includes modern date pickers, real-time validation, consent checkboxes, guardian fields for minors, and professional tooltips. Right-side content area has overflow scroll with fixed Eye Care logo (py-20 top padding to prevent overlap). **Medical Color Palette**: Progress indicators use Medical Green (#2ecc71) for completed steps, Medical Blue (#0066cc) for current/active steps, and Teal (#17a2b8) for gradients.
- **Extended Profile (Optional)**: Comprehensive patient data collection with collapsible accordion design for better UX. Four sections: Personal Details (Blood Group, Government ID, Occupation, Marital Status with conditional Spouse Name), Present Address, Permanent Address (with "Same as Present" checkbox), and Emergency Contact & Source (Emergency Contact Name + Phone with international format, Source of Patient with conditional Doctor/Referral fields for Name + Phone). First section (Personal Details) open by default, other sections collapsed. Smooth expand/collapse animations with rotating chevron icons. "Skip for now" button for easy navigation. Features 5-step progress indicator showing registration journey (Profile → Verification → Complete Profile → MFA Setup → Complete) with step 3 highlighted as current in Medical Blue.
- **MFA Setup**: Supports Authenticator App (TOTP), SMS OTP, and Email OTP, with progressive setup flows and method-specific UI. Features persistent 5-step progress indicator across all MFA screens (method selection, TOTP setup, SMS setup, Email setup), showing: Profile (completed in Medical Green) → Verification (completed in Medical Green) → Complete Profile (completed in Medical Green) → MFA Setup (current in Medical Blue) → Complete (pending in gray). Progress indicator maintains consistent positioning across all views with compact spacing (text-xs, space-x-1.5) to accommodate 5 steps.
